...
import { createTuiFrontendIdentity } from "../rin-lib/frontend-identity.js";
...
async function startStdTui(
  ...
  const { runtime: sessionRuntime } = await createConfiguredAgentSession({
    ...
    tools: resourceOptions.tools,
    excludeTools: resourceOptions.excludeTools,
    noTools: resourceOptions.noTools,
...
async function startRpcTui(
  ...
  const frontendIdentity = createTuiFrontendIdentity();
