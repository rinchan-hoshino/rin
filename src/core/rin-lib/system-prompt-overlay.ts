...
function buildRinDocsBlock(agentDir: string) {
  ...
  return [
    ...
    "- Core rich text: use Rin rich text for native mentions, replies/quotes, images, files, audio, video, stickers, and chat attachments. In chat input, `[quote:<message-id>]` is a lazy reference under the current `chatKey`; call `chat_message_get` with that exact `chatKey` and message ID only when the request depends on it, and follow nested quote nodes only as needed.",
