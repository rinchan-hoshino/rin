...
export const findNpmCli = async ({env=process.env, platform=process.platform, node=process.execPath}={}) => {
  ...
  for (const dir of (env.PATH || '').split(platform === 'win32' ? ';' : ':').filter(Boolean)) candidates.push(join(dir, 'node_modules/npm/bin/npm-cli.js'));
  for (const candidate of candidates.filter(Boolean)) if (await exists(candidate)) return candidate;
  throw new Error('Rin requires npm from Node.js 24 or newer. Install the official Node.js 24+ package so node and npm are both on PATH, then retry.');
};
...
async function addCommandPath(binDir, platform=process.platform, env=process.env, userHome=homedir()) {
  ...
  const file = join(userHome, env.SHELL?.endsWith('/zsh') ? '.zshrc' : '.profile');
...
export async function install({home, repository=REPOSITORY, binDir, platform=process.platform, env=process.env, userHome=env.HOME || env.USERPROFILE || homedir()}={}) {
  home ||= installHome(env, platform);
  binDir ||= platformBin(platform, {...env, HOME:userHome, USERPROFILE:userHome});
  ...
  return withInstallLock(home, async () => {
    ...
    const service = createService({home, userHome, node: process.execPath, platform, env: {...env, PATH: [binDir, dirname(process.execPath), env.PATH || ''].filter(Boolean).join(platform === 'win32' ? ';' : ':')}, serviceId});
    ...
    await addCommandPath(binDir, platform, env, userHome);
    return {home, binDir, release: candidate.sha, serviceId};
  });
}

if (process.argv[1]) Promise.all([realpath(process.argv[1]), realpath(fileURLToPath(import.meta.url))]).then(([actual, expected]) => {
  if (actual !== expected) return;
  return install().then(result => console.log(`Rin installed in ${result.home}. Open a new terminal, then run rin start after configuring the guided prompt.`));
