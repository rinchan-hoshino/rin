...
`main` 维护 Codex 版本；旧 Pi 版本及完整历史保留在 [`legacy/pi`](https://github.com/rinchan-hoshino/rin/tree/legacy/pi)。新版不加载旧 Rin、Pi、旧 daemon 或旧扩展运行时。安装和更新只提供 Git 版，不发布 npm 包。旧版用户可以直接把[Agent 迁移执行手册原文](https://raw.githubusercontent.com/rinchan-hoshino/rin/main/docs/legacy-migration.md)交给代理执行；手册包含两代源码入口、默认安装目录和迁移步骤。[仓库内阅读](docs/legacy-migration.md)。
...
- **聊天桥模块**：Discord、Telegram、QQ 官方、独立 OneBot v11 适配器。将准入后的消息投递到已有 Codex 任务，转发公开文字、明确附件与生成图片；按平台能力编辑进度或发送消息快照。
