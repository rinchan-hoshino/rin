...
**Rin 是一个住在你电脑里的个人 AI 助手：它会记住重要信息，并在日常使用中变得更懂你。**

它不是又一个普通聊天窗口。Rin 会在不同会话之间保持同一个助手身份，在你允许时使用本地工具，并把有用经验沉淀成记忆或可复用技能。

Rin 也由 Rin 自己参与构建。这个项目一直使用 Rin 来规划、编辑、审阅、翻译和维护仓库，所以“自举式开发”不是口号，而是产品自身的一部分验证。

## 为什么试试 Rin

- **上手快：** 安装后运行 `rin`，直接用自然语言交流。
- **少重复解释：** Rin 可以跨会话记住长期事实、偏好、项目和常用要求。
- **让经验累积：** 重复工作可以变成记忆、提示和技能，不需要你自己设计一整套 agent 系统。
- **本地、可检查：** Rin 运行在你的机器上，并尽量清楚展示它使用的工具、文件和配置。
- **一个助手，多处入口：** 终端、GUI、自动化和接入的聊天应用都可以连接到同一个 Rin。
...
Rin 仍处在早期阶段。请预期粗糙边缘、文档缺口、不稳定行为和偶发破坏性变更。
