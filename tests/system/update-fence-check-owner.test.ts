...
const sandboxRoot = await fs.mkdtemp(
  path.join(os.tmpdir(), "rin-update-fence-owner-"),
);
const sandbox = await createTestSandbox(sandboxRoot);
assert.ok(sandbox.env.TMPDIR);
...
const updateFenceCheck = await import(pathToFileURL(entryPath).href);

test("update fence entrypoint validates arguments and executes the sandboxed check", async () => {
  await assert.rejects(
    () => updateFenceCheck.main([]),
    /rin_update_fence_check_args_missing/,
  );
  await updateFenceCheck.main([sandbox.agentDir, "owner"]);
  await assert.rejects(
    () => execFileAsync(process.execPath, [entryPath], { env: sandbox.env }),
    (error: any) => {
      assert.equal(error.code, 1);
      assert.match(error.stderr, /maintenance fence/i);
      return true;
    },
  );
});
