...
test("prompt, prompted, initialized, and startup transitions persist their contract", async () => {
  await withAgentDir(async (agentDir) => {
    ...
    assert.equal(onboarding.buildOnboardingPrompt("auto"), manualPrompt);
    assert.match(manualPrompt, /initialization\.md/);
    assert.match(manualPrompt, /completed state is false/);
