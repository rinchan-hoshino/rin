...
test("chat main wakes configured tasks for record-only chat messages", async () => {
  const tempRoot = "/home/rin/tmp";
  await fs.mkdir(tempRoot, { recursive: true });
  const agentDir = await fs.mkdtemp(
    path.join(tempRoot, "rin-chat-main-queue-"),
  ...
  try {
        ...
        chat: {
          turnPolicy: {
            byChatKey: {
              "telegram/1:2": {
                mode: "record_only",
                wakeTaskId: "cron_record_only_wake",
              },
            },
          },
        },
    ...
    const script = `
      ...
      const server = net.createServer((socket) => {
        ...
        socket.on("data", (chunk) => {
          ...
          while ((index = buffer.indexOf("\\n")) >= 0) {
            ...
            wakeCommands.push(command);
            socket.write(JSON.stringify({ type: "response", id: command.id, command: command.type, success: true, data: { task: { id: command.taskId } } }) + "\\n");
          }
        });
      });
      ...
      const { app } = await mainMod.startChatBridge();
      app.bots.push({ platform: "telegram", selfId: "1", async sendMessage() { return ["sent"]; } });
      ...
      app.emit("message", {
        ...
        channelId: "2",
        userId: "owner-1",
        messageId: "m-record-only-wake",
        isDirect: true,
        content: "wake please",
        stripped: { content: "wake please" },
        elements: [h.createChatRuntimeH().text("wake please")],
      });

      const deadline = Date.now() + 5000;
      while (Date.now() < deadline && wakeCommands.length < 1) {
        await new Promise((resolve) => setTimeout(resolve, 100));
      }
