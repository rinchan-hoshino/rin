...
const {
  ...
  MAX_PENDING_TERMINAL_EVENTS_PER_SESSION,
  MAX_PENDING_TERMINAL_SESSIONS,
  pendingTerminalTurnEventCount,
  pendingTurnEventsStatePath,
...
test("overlapping prompt responses preserve the active lifecycle owner", async () => {
  for (const protocol of ["legacy", "versioned"]) {
    for (const secondResponseSuccess of [true, false]) {
      ...
      const pendingTerminal = JSON.parse(
        await fs.readFile(pendingEventsPath, "utf8"),
      ).eventsBySessionFile?.[sessionFile]?.[0];
      assert.equal(pendingTerminal?.requestTag, ownerTag);
      assert.equal(
        pool.acknowledgePendingTerminalTurnEvent(connection, {
          sessionFile,
          terminalEventId: pendingTerminal.terminalEventId,
        }),
        false,
      );
      assert.equal(
...
`,
  ...
  const pendingAfterReplay = JSON.parse(
    await fs.readFile(pendingEventsPath, "utf8"),
  );
  assert.equal(
    pendingAfterReplay.eventsBySessionFile?.[sessionFile]?.[0]?.requestTag,
    replayed?.requestTag,
  );
  ...
  assert.equal(
    pool.acknowledgePendingTerminalTurnEvent(connection, {
      sessionFile,
      terminalEventId: replayed?.terminalEventId,
      requestTag: replayed?.requestTag,
    }),
    true,
  );
  const pendingAfterAck = JSON.parse(
    await fs.readFile(pendingEventsPath, "utf8"),
  );
  assert.equal(pendingAfterAck.eventsBySessionFile?.[sessionFile], undefined);

  pool.destroyAll();
  await fs.rm(dir, { recursive: true, force: true });
});

test("pending terminal events retain later turns when an earlier ACK is missing", async () => {
  const dir = await makeTempDir("rin-pending-terminal-queue-");
  const sessionFile = path.join(dir, "session.jsonl");
  rememberPendingTerminalTurnEvent(dir, {
    type: "rpc_turn_event",
    event: "complete",
  ...
  const normalizedIdEvent = rememberPendingTerminalTurnEvent(dir, {
    type: "rpc_turn_event",
    event: "complete",
    terminalEventId: "  terminal-normalized  ",
    requestTag: "turn-normalized",
    sessionFile,
  });
  assert.equal(normalizedIdEvent?.terminalEventId, "terminal-normalized");

  let persisted = JSON.parse(
    await fs.readFile(pendingTurnEventsStatePath(dir), "utf8"),
  );
  assert.deepEqual(
    persisted.eventsBySessionFile[sessionFile].map(
      (event: any) => event.terminalEventId,
    ),
    ["terminal-1", "terminal-2", "terminal-normalized"],
  );
  ...
  assert.equal(
    acknowledgePendingTerminalTurnEvent(
      dir,
      { sessionFile },
      { terminalEventId: "terminal-1", requestTag: "turn-1" },
    ),
    true,
  );
  assert.equal(
  ...
  assert.deepEqual(
    ...
    ["terminal-2", "terminal-normalized"],
  );

  const identicalPayload = {
    type: "rpc_turn_event",
    event: "complete",
    requestTag: "identical-content",
    sessionFile,
    finalText: "same final",
  };
  const identicalFirst = rememberPendingTerminalTurnEvent(
    dir,
    identicalPayload,
  );
  const identicalSecond = rememberPendingTerminalTurnEvent(
    dir,
    identicalPayload,
  );
  assert.equal(
    identicalFirst?.terminalEventId,
    identicalSecond?.terminalEventId,
  );
  rememberPendingTerminalTurnEvent(dir, {
    type: "rpc_turn_event",
    event: "complete",
    terminalEventId: "shared-id",
    requestTag: "shared-tag-a",
    sessionFile,
  });
  rememberPendingTerminalTurnEvent(dir, {
    type: "rpc_turn_event",
    event: "complete",
    terminalEventId: "shared-id",
    requestTag: "shared-tag-b",
    sessionFile,
  });
  rememberPendingTerminalTurnEvent(dir, {
    type: "rpc_turn_event",
    event: "complete",
    terminalEventId: "space-vs-absent-id",
    requestTag: "   ",
    sessionFile,
  });
  rememberPendingTerminalTurnEvent(dir, {
  ...
  assert.equal(persisted.eventsBySessionFile[sessionFile].length, 7);
  assert.deepEqual(
    persisted.eventsBySessionFile[sessionFile]
      .filter((event: any) => event.terminalEventId === "shared-id")
      .map((event: any) => event.requestTag),
    ["shared-tag-a", "shared-tag-b"],
  );
  assert.equal(
    persisted.eventsBySessionFile[sessionFile].filter(
      (event: any) => event.terminalEventId === "space-vs-absent-id",
    ).length,
    2,
  );
  rememberPendingTerminalTurnEvent(dir, {
    type: "rpc_turn_event",
    event: "complete",
    terminalEventId: "queued-untagged-terminal",
    sessionFile,
  });
  assert.equal(
    getPendingTerminalTurnEvent(
  ...
  const untaggedSessionFile = path.join(dir, "untagged.jsonl");
  rememberPendingTerminalTurnEvent(dir, {
    type: "rpc_turn_event",
    event: "complete",
    terminalEventId: "untagged-terminal",
    sessionFile: untaggedSessionFile,
  });
  assert.equal(
    getPendingTerminalTurnEvent(
      dir,
  ...
  for (const [name, requestTag] of [
    ["tagged", "replayed-tag"],
    ["blank", ""],
    ["undefined", undefined],
  ] as const) {
    const replaySessionFile = path.join(dir, `${name}.jsonl`);
    const terminalEventId = `replayed-${name}`;
    const payload = {
      type: "rpc_turn_event",
      event: "complete",
      terminalEventId,
      sessionFile: replaySessionFile,
      ...(requestTag === undefined ? {} : { requestTag }),
    };
    const requestTagIdentity =
      requestTag === undefined
        ? ({ requestTagAbsent: true } as const)
        : { requestTag };
    assert.notEqual(rememberPendingTerminalTurnEvent(dir, payload), false);
    assert.equal(
      acknowledgePendingTerminalTurnEvent(
        dir,
        { sessionFile: replaySessionFile },
        { terminalEventId, ...requestTagIdentity },
      ),
      true,
    );
    assert.equal(rememberPendingTerminalTurnEvent(dir, payload), false);
    assert.equal(
      getPendingTerminalTurnEvent(
        dir,
        { sessionFile: replaySessionFile },
...
test("pending terminal events stop admitting growth at the durable session cap", async () => {
  const dir = await makeTempDir("rin-pending-terminal-cap-");
  const sessionFile = path.join(dir, "bounded-pending.jsonl");
  let overflowResult: unknown = undefined;
  for (
    let index = 0;
    index <= MAX_PENDING_TERMINAL_EVENTS_PER_SESSION;
    index += 1
  ) {
    overflowResult = rememberPendingTerminalTurnEvent(dir, {
      type: "rpc_turn_event",
      event: "complete",
      requestTag: `request-${index}`,
      sessionFile,
      finalText: `terminal-${index}`,
    });
  }

  assert.equal(overflowResult, null);
  assert.equal(
    pendingTerminalTurnEventCount(dir, { sessionFile }),
    MAX_PENDING_TERMINAL_EVENTS_PER_SESSION,
  );
  await fs.rm(dir, { recursive: true, force: true });
});

test("pending terminal events stop admitting new session keys at the global cap", async () => {
  const dir = await makeTempDir("rin-pending-terminal-session-cap-");
  for (let index = 0; index < MAX_PENDING_TERMINAL_SESSIONS; index += 1) {
    const remembered = rememberPendingTerminalTurnEvent(dir, {
      type: "rpc_turn_event",
      event: "complete",
      requestTag: `request-${index}`,
      sessionFile: path.join(dir, `session-${index}.jsonl`),
      finalText: `terminal-${index}`,
    });
    assert.ok(remembered);
  }
  const overflow = rememberPendingTerminalTurnEvent(dir, {
    type: "rpc_turn_event",
    event: "complete",
    requestTag: "request-overflow",
    sessionFile: path.join(dir, "session-overflow.jsonl"),
    finalText: "overflow",
  });
...
test("pending terminal state rejects structurally corrupt valid JSON", async () => {
  const dir = await makeTempDir("rin-pending-terminal-corrupt-");
  const statePath = pendingTurnEventsStatePath(dir);
  await fs.mkdir(path.dirname(statePath), { recursive: true });
  const invalidStates = [
    {},
    { schemaVersion: 99, eventsBySessionFile: {} },
    { schemaVersion: 3, eventsBySessionFile: "bad", acknowledgedEvents: [] },
    { schemaVersion: 3, eventsBySessionFile: {}, acknowledgedEvents: "bad" },
  ];
  for (const invalidState of invalidStates) {
    await fs.writeFile(statePath, JSON.stringify(invalidState), "utf8");
    assert.throws(() =>
      rememberPendingTerminalTurnEvent(dir, {
        type: "rpc_turn_event",
        event: "complete",
        requestTag: "request-corrupt",
        sessionFile: path.join(dir, "corrupt.jsonl"),
        finalText: "must fail closed",
      }),
    );
  }
  await fs.rm(dir, { recursive: true, force: true });
...
test("pending terminal state rejects oversized valid pending collections", async () => {
  const dir = await makeTempDir("rin-pending-terminal-oversized-");
  const statePath = pendingTurnEventsStatePath(dir);
  await fs.mkdir(path.dirname(statePath), { recursive: true });
  const terminal = (sessionFile: string, index: number) => ({
    type: "rpc_turn_event",
    event: "complete",
    terminalEventId: `terminal-${index}`,
    requestTag: `request-${index}`,
    sessionFile,
    finalText: `final-${index}`,
  });
  const oversizedSessions = Object.fromEntries(
    Array.from({ length: MAX_PENDING_TERMINAL_SESSIONS + 1 }, (_, index) => {
      const sessionFile = path.join(dir, `oversized-${index}.jsonl`);
      return [sessionFile, [terminal(sessionFile, index)]];
    }),
  );
  const oversizedEventsSession = path.join(dir, "oversized-events.jsonl");
  const oversizedStates = [
    oversizedSessions,
    {
      [oversizedEventsSession]: Array.from(
        { length: MAX_PENDING_TERMINAL_EVENTS_PER_SESSION + 1 },
        (_, index) => terminal(oversizedEventsSession, index),
      ),
    },
  ];
  for (const eventsBySessionFile of oversizedStates) {
    await fs.writeFile(
      statePath,
      JSON.stringify({
        schemaVersion: 3,
    ...
    assert.throws(() =>
      rememberPendingTerminalTurnEvent(dir, {
        ...
        sessionFile: path.join(dir, "current.jsonl"),
        finalText: "must fail closed",
      }),
    );
  }
...
test("pending terminal ACK tombstones remain bounded", async () => {
  const dir = await makeTempDir("rin-pending-terminal-bounded-");
  const sessionFile = path.join(dir, "bounded.jsonl");
  const statePath = pendingTurnEventsStatePath(dir);
  const acknowledgedEvents = Array.from({ length: 4_100 }, (_, index) => ({
    terminalEventId: `terminal-${index}`,
    sessionFile,
    requestTag: `request-${index}`,
  ...
  await fs.writeFile(
    statePath,
    `${JSON.stringify({ schemaVersion: 3, eventsBySessionFile: {}, acknowledgedEvents })}\n`,
    "utf8",
  );
  ...
  rememberPendingTerminalTurnEvent(dir, {
    type: "rpc_turn_event",
    event: "complete",
    terminalEventId: "terminal-current",
    requestTag: "request-current",
    sessionFile,
  });

  const persisted = JSON.parse(await fs.readFile(statePath, "utf8"));
  assert.equal(persisted.acknowledgedEvents.length, 4_096);
  assert.equal(persisted.acknowledgedEvents[0].terminalEventId, "terminal-4");
  assert.equal(
    persisted.acknowledgedEvents.at(-1).terminalEventId,
    "terminal-4099",
  );
  await fs.rm(dir, { recursive: true, force: true });
});
