...
test("chat terminal backlog is reconciled without waiting for new ingress", () => {
  ...
  assert.match(
    chatMain,
    /terminalRecord\?\.terminalId[\s\S]*controllers\.get\(chatKey\)[\s\S]*ownsAuthoritativeTerminalProjection\(terminal\)[\s\S]*activeController\.driver\.projectAuthoritativeTerminal[\s\S]*terminal-reconcile:\$\{terminalId\}[\s\S]*affectChatBinding: false[\s\S]*useChatFrontendIdentity: false[\s\S]*restoreSession: false[\s\S]*recoverTerminals: false[\s\S]*projectAuthoritativeTerminal\(terminal\)/,
  );
...
test("chat lifecycle settlement is independent of restart and lease timing", () => {
  ...
  assert.doesNotMatch(
    chatMain,
    /handleTurnFailure|chat turn failed|frontend_turn_interrupted|rin_worker_exit|rin_prompt_outcome_indeterminate/,
  );
  assert.doesNotMatch(
    `${chatMain}\n${inboxDrain}\n${inbox}`,
    /failClaimedChatInboxItem|chat_inbound_still_owned/,
  );
  assert.match(
    chatMain,
    /chat inbox worker detached without terminal[\s\S]*releaseClaimedInboxJob\(job\)/,
  );
