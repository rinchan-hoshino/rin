...
test("chat terminal backlog is reconciled without waiting for new ingress", () => {
  ...
  assert.match(
    chatMain,
    /chatTerminalRecordId[\s\S]*terminal-reconcile:\$\{terminalId\}[\s\S]*affectChatBinding: false[\s\S]*recoverTerminals: false[\s\S]*projectAuthoritativeTerminal\(terminal\)/,
  );
});
