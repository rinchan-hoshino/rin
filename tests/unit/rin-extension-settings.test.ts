...
test("background extension configs reject disabled or incomplete entries and normalize accepted packages", () => {
  assert.deepEqual(
    extensionSettings.listRinBackgroundExtensionConfigs(null),
    [],
  );
  assert.deepEqual(
    extensionSettings.listRinBackgroundExtensionConfigs({
      rinExtensions: { backgroundServices: "not an array" },
    }),
    [],
  );
  ...
  const configs = extensionSettings.listRinBackgroundExtensionConfigs(
    settings,
    {
      cwd: "/unused",
    },
  ...
  assert.equal(
    settings.rinExtensions.backgroundServices[4].config.nested.enabled,
    true,
  );
});
