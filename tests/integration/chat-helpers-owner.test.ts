...
test("chat helpers select only own unsessioned quote content", () => {
  const own = {
    userId: "owner",
    user: { id: "owner" },
    quote: {
  ...
  assert.equal(helpers.pickUnsessionedOwnQuoteText({ session: own }), "quoted");
  assert.equal(
    helpers.pickUnsessionedOwnQuoteText({
      session: own,
      linkedSessionFile: "/tmp/s.jsonl",
    }),
    "",
  );
  assert.equal(helpers.pickUnsessionedOwnQuoteText({ session: {} }), "");
  assert.equal(
    helpers.pickUnsessionedOwnQuoteText({
      session: { quote: { messageId: "q" } },
    }),
    "",
  );
  assert.equal(
    helpers.pickUnsessionedOwnQuoteText({
      session: {
        userId: "owner",
        quote: { messageId: "q", userId: "guest", content: "no" },
      },
    }),
    "",
  );
  assert.equal(
    helpers.pickUnsessionedOwnQuoteText({
      session: { userId: "owner", quote: { messageId: "q" } },
      linked: { userId: "owner", strippedContent: "linked quote" },
    }),
    "linked quote",
  );
  assert.equal(
    helpers.pickUnsessionedOwnQuoteText({
      session: { userId: "owner", quote: { messageId: "q" } },
      linked: { userId: "owner", rawContent: "raw linked quote" },
    }),
    "raw linked quote",
  );
  assert.equal(helpers.prependQuoteTextToPromptBody("body", ""), "body");
  assert.equal(helpers.prependQuoteTextToPromptBody("", "quote"), "quote");
  assert.equal(
    helpers.prependQuoteTextToPromptBody("body", "quote"),
    "quote\n\nbody",
  );
});
