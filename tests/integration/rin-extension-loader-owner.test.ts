...
const fullExtension = `export default async function ownerExtension(api) {
  ...
  api.on("", () => "ignored");
  api.on("ignored", "not-a-function");
  api.registerTool({ name: " owner_tool ", description: api.runtimeRoot });
  api.registerTool({ name: "" });
  api.registerCommand(" owner-command ", { description: api.dataDir });
  api.registerCommand("", {});
  api.registerShortcut(" ctrl+x ", { description: api.cwd });
  api.registerShortcut("", {});
  api.registerFlag(" owner-flag ", { default: "enabled" });
  api.registerFlag("", {});
  api.registerMessageRenderer(" owner-message ", () => "rendered");
  api.registerMessageRenderer("", () => "ignored");
  api.registerMessageRenderer("ignored", "not-a-function");
  api.registerProvider(); api.unregisterProvider(); api.registerBackgroundService();
  api.registerChatAdapter(); api.registerMemoryProvider(); api.runAsync();
  ...
  api.logger.info(); api.logger.warn(); api.logger.error();
  api.heartbeat.appendInfo(); api.events.on(); api.events.off(); api.events.emit();
  await api.exec(); await api.setModel();
...
function fakeRuntime(configured: string[]) {
  ...
  class BaseLoader {
    ...
    agentDir: string;
    settingsManager: any;
    extensionsResult = { extensions: [] as any[], errors: [] as any[] };
...
test("Rin resource loader resolves file, directory, package, nested, and TypeScript extensions", async () => {
  await withTempDir(async (root) => {
    ...
    assert.equal(result.extensions.length, 7);
...
test("Rin resource loader honors noExtensions while retaining temporary CLI extensions", async () => {
  await withTempDir(async (root) => {
    ...
    assert.equal(loader.getExtensions().errors.length, 2);
    assert.equal(
