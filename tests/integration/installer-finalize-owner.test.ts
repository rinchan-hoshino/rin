...
async function withSandbox(run: (root: string) => Promise<void>) {
  ...
  scenario.agentDir = path.join(root, "agent");
  scenario.socketPath = path.join(root, "runtime", "owner.sock");
  scenario.daemonReady = true;
  await Promise.all([
    fs.mkdir(scenario.sourceRoot, { recursive: true }),
    fs.mkdir(scenario.agentDir, { recursive: true }),
    fs.mkdir(path.dirname(scenario.socketPath), { recursive: true }),
  ]);
  try {
    await run(root);
  } finally {
