...
test("chat outbox query helpers expose active fences and terminal history", async () => {
  await withTempDir(async (dir) => {
    assert.equal(outbox.normalizeChatOutboxPayload(null), null);
    assert.equal(outbox.normalizeChatOutboxPayload("owner"), null);
    assert.equal(
      outbox.normalizeChatOutboxPayload({
        parts: [{ type: "text", text: "missing chat" }],
      }),
      null,
    );
    assert.equal(
    ...
    const fence = {
      ...
      ownerEpoch: "epoch-owner",
      attempt: 1,
    };
    assert.equal(outbox.getActiveChatOutboxTurnFence(), undefined);
    assert.equal(
      outbox.runWithChatOutboxTurnFence(fence, () =>
        outbox.getActiveChatOutboxTurnFence(),
      ),
      fence,
    );
    assert.equal(
    ...
    assert.throws(
      () =>
        outbox.enqueueChatOutboxPayload(dir, payload("wrong-agent"), {
          turnFence: { ...fence, agentDir: `${dir}-wrong` },
        }),
      /chat_turn_fence_lost/,
    );
    assert.throws(
      () =>
        outbox.enqueueChatOutboxPayload(dir, payload("missing-turn"), {
          turnFence: fence,
        }),
      /chat_turn_fence_lost/,
    );

    const dedupeId = outbox.enqueueChatOutboxPayload(
      dir,
      payload("dedupe-row"),
      { idempotencyKey: "owner-dedupe" },
    );
    assert.equal(
      outbox.readChatOutboxItemById(dir, dedupeId).item.idempotencyKey,
      "owner-dedupe",
    );

    const malformedId = outbox.enqueueChatOutboxPayload(
      dir,
      payload("malformed-row"),
    );
    database
      .openChatDatabase(dir)
      .prepare(
        `UPDATE outbox SET payload_json = 'not-json' WHERE outbox_id = ?`,
      )
      .run(malformedId);
    assert.equal(outbox.readChatOutboxItemById(dir, malformedId), null);

    const id = outbox.enqueueChatOutboxPayload(dir, payload("history"), {
      postDelivery: {
        markProcessed: {
          chatKey: fence.chatKey,
    ...
    assert.equal(
      outbox.writeChatOutboxItem(dir, {
        ...
        attempts: 2,
        ownerEpoch: "",
        lastError: "owner previous error",
        failureKind: "retryable",
        deliveryUnconfirmed: true,
        deliveryResult: ["provider-owner"],
        dispatchStartedAt: timestamp,
        deliveredAt: timestamp,
      }),
      true,
    );
    assert.equal(outbox.listCommittedChatOutboxItems(dir).length, 1);
    assert.equal(outbox.listChatOutboxHistoryItems(dir, "delivered").length, 1);
    assert.equal(
      outbox.markChatOutboxPostDeliveryApplied(dir, id, timestamp),
      true,
    );
    assert.equal(
      outbox.listChatOutboxHistoryItems(dir, "delivered")[0]
        .postDeliveryAppliedAt,
      timestamp,
    );
    assert.deepEqual(outbox.listCommittedChatOutboxItems(dir), []);

    const failedId = outbox.enqueueChatOutboxPayload(dir, payload("failed"));
    const failed = outbox.readChatOutboxItemById(dir, failedId).item;
    assert.equal(
      outbox.writeChatOutboxItem(dir, {
        ...failed,
        status: "failed",
    ...
    assert.equal(
      outbox.listChatOutboxHistoryItems(dir, "failed")[0].failedAt,
      timestamp,
    );
  });
});
