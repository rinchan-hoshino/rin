...
test("chat database cutover migrates v8 run ownership to interrupted delivery-only turns", async () => {
  await withTempDir(async (agentDir) => {
    ...
    const unfenced = chatInbox.enqueueChatInboxItem(agentDir, {
      chatKey: "discord/1:2",
      messageId: "platform-v8-unfenced",
      session: {
        platform: "discord",
        selfId: "1",
        channelId: "2",
        messageId: "platform-v8-unfenced",
        content: "interrupt this unfenced turn safely",
        stripped: { content: "interrupt this unfenced turn safely" },
      },
      elements: [
        {
          type: "text",
          attrs: { content: "interrupt this unfenced turn safely" },
        },
      ],
    }).item;
    const claimed = chatInbox.claimChatInboxItem(agentDir, queued.itemId);
    const unfencedClaimed = chatInbox.claimChatInboxItem(
      agentDir,
      unfenced.itemId,
    );
    assert.ok(claimed);
    assert.ok(unfencedClaimed);
      ...
      migrated
        .prepare(
          `SELECT delivery_kind, state
             FROM outbox WHERE turn_id = ?`,
        )
        .get(unfenced.itemId),
      { delivery_kind: "error", state: "queued" },
    );
    assert.equal(
      migrated
        .prepare(
