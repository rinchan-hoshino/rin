...
test("discord adapter owns SDK lifecycle, native wrappers, indicators, and ingress", async () => {
  ...
  await withTempDir(async (directory) => {
    ...
    assert.deepEqual(owner.discordClients[0].options, {
      intents: [1, 2, 4, 8],
      partials: ["channel"],
    });
...
test("lark adapter owns SDK startup, event settlement, native APIs, and parsing fallbacks", async () => {
  ...
  await withTempDir(async (directory) => {
    ...
    assert.equal(await bot.getGuildMemberCount("chat"), 3);
    assert.equal(await bot.getGuildMemberCount(""), 0);
...
test("discord and slack preserve fallback, authorization, and delivery error branches", async () => {
  ...
  await withTempDir(async (directory) => {
    ...
    assert.equal(
      await discordBot.hasOnlyOwnerUsers("missing", ["owner"]),
      false,
    );
    assert.equal(await discordBot.hasOnlyOwnerUsers("child", []), false);
    assert.equal(await discordBot.hasOnlyOwnerUsers("child", ["owner"]), true);
    overwrites.set("unknown", { id: "unknown", allow: { bitfield: 1024n } });
    assert.equal(await discordBot.hasOnlyOwnerUsers("child", ["owner"]), false);
    overwrites.delete("unknown");
    channel.guild.ownerID = "stranger";
    assert.equal(await discordBot.hasOnlyOwnerUsers("child", ["owner"]), false);
    channel.guild.ownerID = "owner";
    channel.guild.roles.everyone.id = "";
    channel.guild.id = "";
    assert.equal(await discordBot.hasOnlyOwnerUsers("child", ["owner"]), false);
    channel.guild.id = "everyone";
    channel.guild.roles.everyone.id = "everyone";
    ...
    const eventFetch = globalThis.fetch;
    try {
      globalThis.fetch = (async () =>
        new Response(Buffer.from("image"))) as any;
    ...
    assert.deepEqual(slackApp.records.at(-1)[1].quote, { messageId: "thread" });
...
test("lark adapter preserves recovery, forward, resource, and image failure branches", async () => {
  ...
  await withTempDir(async (directory) => {
    ...
    assert.equal(await lark.bot.getGuildMemberCount("chat"), 3);
    client.im.chat.get = async () => ({
      data: { user_count: "bad", bot_count: 2 },
    });
    assert.equal(await lark.bot.getGuildMemberCount("chat"), 0);
    ...
    try {
      globalThis.fetch = (async () =>
        new Response("no", { status: 404 })) as any;
      await assert.rejects(
...
test("adapter platform permutations keep lifecycle, identity, and ingress fallbacks observable", async () => {
  ...
  await withTempDir(async (directory) => {
    ...
    assert.equal(await discord.bot.hasOnlyOwnerUsers("chat", ["owner"]), false);
    ...
    const minecraftLog = logger();
    const minecraft = new adapters.MinecraftAdapter(
      minecraftApp,
      directory,
      { url: "ws://minecraft", accessToken: "access", selfId: "" },
      minecraftLog,
    ) as any;
    assert.equal(minecraft.bot.selfId, "minecraft");
    owner.wsOpenError = new Error("open failed");
    await assert.rejects(minecraft.connect(), /open failed/);
    owner.wsOpenError = undefined;
    ...
    await assert.rejects(minecraft.bot.internal.broadcast("x"), /api failed/);
    await minecraft.handleSocketMessage(
      JSON.stringify({ event_name: "Unknown", player: { nickname: "Owner" } }),
    );
    await minecraft.handleSocketMessage(
      JSON.stringify({ event_name: "PlayerChatEvent", player: {} }),
    );
    await minecraft.handleSocketMessage(
      JSON.stringify({
        event_name: "PlayerChatEvent",
        timestamp: "bad",
        player: { nickname: "Owner" },
        message: "plain",
      }),
    );
    assert.equal(minecraftApp.records.at(-1)[1].channelId, "minecraft");
    assert.equal(
      minecraft.buildSession({
        event_name: "PlayerChatEvent",
        player: { uuid: "u" },
      }).elements.length,
      0,
    ...
    const pendingRejected = new Promise((_resolve, reject) => {
      minecraft.pending.set("pending", {
        resolve() {},
        reject,
        timer: setTimeout(() => {}, 10000),
      });
    });
    minecraft.rejectPending(new Error("disconnected"));
    await assert.rejects(pendingRejected, /disconnected/);
    minecraft.ws = {
      close() {
        throw new Error("close failed");
      },
    };
    minecraft.loopPromise = Promise.reject(new Error("loop failed"));
    await minecraft.stop();
    assert.equal(minecraft.bot.status, 0);
  });
});

test("minecraft adapter owns WebSocket lifecycle, RPC settlement, ingress, and send routing", async () => {
  resetOwner();
  await withTempDir(async (directory) => {
    const targetApp = app();
    const log = logger();
    const missing = new adapters.MinecraftAdapter(
      targetApp,
      directory,
      {},
      log,
    );
    await assert.rejects((missing as any).connect(), /minecraft_url_required/);

    const adapter = new adapters.MinecraftAdapter(
      targetApp,
      directory,
      {
        url: "ws://minecraft.owner",
        token: "secret",
    ...
    assert.equal(
      owner.webSockets[0].options.headers["x-self-name"],
      "Owner Server",
    );

    assert.equal((await bot.internal.broadcast("hello")).status, "SUCCESS");
    assert.equal(
      (await bot.internal.sendPrivateMessage("Owner", "hello")).status,
      "SUCCESS",
    );
    await bot.internal.sendRconCommand("list");
    await bot.internal.title("Owner", "Title", "Sub");
    await bot.internal.actionBar("Owner", "Action");

    owner.webSockets[0].emit(
      "message",
      Buffer.from(
        JSON.stringify({
          event_name: "PlayerChatEvent",
          ...
          message: "@rin hello",
          player: { uuid: "uuid", nickname: "Owner" },
        }),
      ),
    );
    await new Promise((resolve) => setImmediate(resolve));
    const inbound = targetApp.records.find(([name]) => name === "message")?.[1];
    assert.equal(inbound.userId, "uuid");
    assert.equal(inbound.stripped.content, "hello");

    const direct = await bot.sendMessage("private:Owner", [
      { type: "at", attrs: { id: "Owner", name: "Owner" }, children: [] },
      { type: "text", attrs: { content: " hello" }, children: [] },
    ]);
    const broadcast = await bot.sendMessage("Owner Server", [
      { type: "text", attrs: { content: "everyone" }, children: [] },
    ]);
    assert.equal(direct.length, 1);
    assert.equal(broadcast.length, 1);
    await assert.rejects(
      bot.sendMessage("Owner Server", []),
      /send_message_empty/,
    );

    assert.equal((adapter as any).buildSession({ event_name: "other" }), null);
    assert.equal(
      (adapter as any).buildSession({
    ...
    await (adapter as any).handleSocketMessage("not-json");
    await (adapter as any).handleSocketMessage(JSON.stringify({}));

    await adapter.stop();
    assert.equal(bot.status, 0);
    await assert.rejects(bot.internal.broadcast("offline"), /not_connected/);

    const running = new adapters.MinecraftAdapter(
      targetApp,
      directory,
      { endpoint: "ws://minecraft.owner" },
      log,
    );
    await running.start();
    await new Promise((resolve) => setImmediate(resolve));
    await running.stop();
    assert.equal((running as any).loopPromise, null);
  });
});
