import assert from "node:assert/strict";
import fs from "node:fs/promises";
import os from "node:os";
import path from "node:path";
import test from "node:test";
import BetterSqlite3 from "better-sqlite3";

import { importBuiltModule } from "../support/import-built-module.js";

const catalog = await importBuiltModule<
  typeof import("../../src/core/session/catalog.js")
>("dist/core/session/catalog.js");

async function withSessionDir(run: (directory: string) => Promise<void>) {
  const directory = await fs.mkdtemp(
    path.join(os.tmpdir(), "rin-catalog-owner-"),
  );
  try {
    await run(directory);
  } finally {
    await fs.rm(directory, { recursive: true, force: true });
  }
}

function sessionEntries(id: string, cwd: string, timestamp: string, name = "") {
  return [
    {
      type: "session",
      id,
      cwd,
      timestamp,
      parentSession: id === "child" ? "/parent.jsonl" : "",
    },
    ...(name ? [{ type: "session_info", name }] : []),
    {
      type: "message",
      timestamp,
      message: {
        role: "user",
        content: [{ type: "text", text: `hello ${id}` }, { type: "image" }],
      },
    },
    {
      type: "message",
      timestamp: new Date(Date.parse(timestamp) + 1000).toISOString(),
      message: { role: "assistant", content: `answer ${id}` },
    },
    {
      type: "message",
      timestamp: "bad",
      message: { role: "toolResult", content: "ignored" },
    },
  ];
}

async function writeSession(filePath: string, entries: any[]) {
  await fs.writeFile(
    filePath,
    `${entries.map((entry) => JSON.stringify(entry)).join("\n")}\n`,
  );
}

test("session catalog summarizes typed entries and rejects invalid records", () => {
  const fallback = new Date("2026-07-15T00:00:00.000Z");
  assert.equal(
    catalog.summarizeSessionEntries("/tmp/no-header.jsonl", [
      { type: "unknown" },
    ]),
    null,
  );
  const summary = catalog.summarizeSessionEntries(
    "/tmp/child.jsonl",
    sessionEntries("child", "/work/owner", "2026-07-15T01:00:00.000Z", "Named"),
    fallback,
  );
  assert.ok(summary);
  assert.equal(summary.id, "child");
  assert.equal(summary.name, "Named");
  assert.equal(summary.firstMessage, "hello child");
  assert.equal(summary.messageCount, 3);
  assert.equal(summary.parentSessionPath, "/parent.jsonl");
  assert.match(summary.allMessagesText, /answer child/);

  const fallbackSummary = catalog.summarizeSessionEntries(
    "/tmp/fallback.jsonl",
    [{ type: "session", timestamp: "bad" }],
    fallback,
  );
  assert.equal(fallbackSummary?.id, "/tmp/fallback.jsonl");
  assert.equal(fallbackSummary?.firstMessage, "(no messages)");
  assert.equal(fallbackSummary?.modified.toISOString(), fallback.toISOString());
});

test("session catalog preserves complete mixed message search text", () => {
  const timestamp = "2026-07-15T01:00:00.000Z";
  const entries: any[] = [
    { type: "session", id: "mixed", cwd: "/work/mixed", timestamp },
    { type: "session_info", name: " " },
    null,
    {
      type: "message",
      timestamp: "bad",
      message: { role: "user" },
    },
    {
      type: "message",
      timestamp: "bad",
      message: { role: "user", content: [] },
    },
    {
      type: "message",
      timestamp: "",
      message: { role: "system", content: "ignored" },
    },
    {
      type: "message",
      timestamp: "bad",
      message: { role: "toolResult", content: "ignored" },
    },
    {
      type: "message",
      message: {
        role: "assistant",
        timestamp: Date.parse(timestamp) + 10,
        content: [{ type: "image" }, { type: "text", text: "answer" }],
      },
    },
    ...Array.from({ length: 10 }, (_, index) => ({
      type: "message",
      timestamp,
      message: { role: "user", content: `${index}-${"x".repeat(2500)}` },
    })),
    null,
  ];
  const summary = catalog.summarizeSessionEntries("/tmp/mixed.jsonl", entries);
  assert.ok(summary);
  assert.equal(summary.name, undefined);
  assert.equal(summary.firstMessage.length, 2502);
  assert.equal(summary.allMessagesText.length > 16_000, true);
  assert.match(summary.allMessagesText, /9-x{2500}$/);
});

test("session catalog summary falls back across sparse header and activity shapes", () => {
  const sparse = catalog.summarizeSessionEntries("/tmp/sparse.jsonl", [
    { type: "session", id: "", cwd: 42 },
    { type: "session_info" },
    {
      type: "message",
      message: { role: "assistant", timestamp: 10, content: "answer" },
    },
    {
      type: "message",
      timestamp: "invalid",
      message: { role: "user", content: "" },
    },
  ]);
  assert.ok(sparse);
  assert.equal(sparse.id, "/tmp/sparse.jsonl");
  assert.equal(sparse.cwd, "");
  assert.equal(sparse.name, undefined);
  assert.equal(sparse.firstMessage, "(no messages)");
  assert.equal(sparse.modified.getTime(), 10);

  const generated = catalog.summarizeSessionEntries("/tmp/generated.jsonl", [
    { type: "session", id: "generated", timestamp: "invalid" },
  ]);
  assert.equal(Number.isFinite(generated?.modified.getTime()), true);
  assert.equal(catalog.summarizeSessionEntries("/tmp/empty.jsonl", []), null);
});

test("session catalog reads files, ignores malformed lines, and preserves order", async () => {
  await withSessionDir(async (directory) => {
    const first = path.join(directory, "first.jsonl");
    const second = path.join(directory, "second.jsonl");
    await writeSession(
      first,
      sessionEntries("first", "/work/a", "2026-07-15T01:00:00.000Z"),
    );
    await fs.writeFile(
      second,
      `not-json\n${JSON.stringify({ type: "session", id: "second", timestamp: "2026-07-15T02:00:00.000Z" })}\n`,
    );
    await fs.writeFile(path.join(directory, "ignored.txt"), "ignored");
    const empty = path.join(directory, "empty.jsonl");
    await fs.writeFile(empty, "");
    await fs.mkdir(path.join(directory, "folder.jsonl"));

    assert.deepEqual(
      await catalog.listSessionRecordFiles(path.join(directory, "missing")),
      [],
    );
    assert.deepEqual(
      (await catalog.listSessionRecordFiles(directory)).sort(),
      [empty, first, second].sort(),
    );
    assert.equal(
      await catalog.readSessionSummary(path.join(directory, "ignored.txt")),
      null,
    );
    assert.equal(
      await catalog.readSessionSummary(path.join(directory, "missing.jsonl")),
      null,
    );
    assert.equal(await catalog.readSessionSummary(empty), null);
    assert.equal((await catalog.readSessionSummary(first))?.id, "first");

    const summaries = await catalog.loadSessionSummaries([
      first,
      "",
      path.join(directory, "missing"),
      second,
    ]);
    assert.deepEqual(
      summaries.map((summary) => summary.id),
      ["first", "second"],
    );
  });
});

test("session catalog rebuilds scoped pages and repairs partial state", async () => {
  await withSessionDir(async (directory) => {
    for (const [id, cwd, timestamp] of [
      ["one", "/work/a", "2026-07-15T01:00:00.000Z"],
      ["two", "/work/b", "2026-07-15T02:00:00.000Z"],
      ["three", "/work/a", "2026-07-15T03:00:00.000Z"],
    ] as const) {
      await writeSession(
        path.join(directory, `${id}.jsonl`),
        sessionEntries(id, cwd, timestamp),
      );
    }

    assert.equal(
      await catalog.tryListSessionCatalogPage({
        sessionDir: directory,
        offset: 0,
        limit: 2,
      }),
      undefined,
    );
    assert.deepEqual(await catalog.rebuildSessionCatalog(directory), {
      checked: 3,
      indexed: 3,
    });
    const firstPage = await catalog.tryListSessionCatalogPage({
      sessionDir: directory,
      offset: 0,
      limit: 2,
    });
    assert.deepEqual(
      firstPage?.sessions.map((session) => session.id),
      ["three", "two"],
    );
    assert.equal(firstPage?.hasMore, true);
    assert.equal(firstPage?.nextOffset, 2);

    const scoped = await catalog.tryListSessionCatalogPage({
      sessionDir: directory,
      cwd: "/work/a",
      offset: 0,
      limit: 10,
    });
    assert.deepEqual(
      scoped?.sessions.map((session) => session.id),
      ["three", "one"],
    );
    assert.equal(scoped?.hasMore, false);

    await catalog.ensureSessionCatalog(directory);
    await fs.rm(
      path.join(directory, ".rin-session-catalog", "v3", "catalog.sqlite"),
    );
    await catalog.ensureSessionCatalog(directory);
    assert.ok(
      await catalog.tryListSessionCatalogPage({
        sessionDir: directory,
        offset: 0,
        limit: 1,
      }),
    );
  });
});

test("session catalog reads large head-tail files and filters malformed tail records", async () => {
  await withSessionDir(async (directory) => {
    const largePath = path.join(directory, "large.jsonl");
    const header = JSON.stringify({
      type: "session",
      id: "large",
      cwd: "/work/large",
      timestamp: "2026-07-15T00:00:00.000Z",
    });
    const filler = JSON.stringify({
      type: "metadata",
      value: "x".repeat(300_000),
    });
    const tail = JSON.stringify({
      type: "message",
      timestamp: "2026-07-15T05:00:00.000Z",
      message: { role: "user", content: "tail owner message" },
    });
    await fs.writeFile(largePath, `${header}\n${filler}\n${tail}\n`);
    const summary = await catalog.readSessionSummary(largePath);
    assert.equal(summary?.id, "large");
    assert.equal(summary?.firstMessage, "tail owner message");

    await catalog.rebuildSessionCatalog(directory);
    await fs.appendFile(
      largePath,
      `${JSON.stringify({
        type: "message",
        timestamp: "2026-07-15T06:00:00.000Z",
        message: { role: "assistant", content: "newest" },
      })}\n`,
    );
    assert.equal(
      catalog.updateSessionCatalogFromSessionManagerSync({
        isPersisted: () => true,
        sessionFile: largePath,
        getSessionDir: () => directory,
      }),
      true,
    );
    assert.equal(
      await catalog.tryListSessionCatalogPage({
        sessionDir: directory,
        offset: 0,
        limit: 10,
      }),
      undefined,
    );
    await catalog.ensureSessionCatalog(directory);
    const page = await catalog.tryListSessionCatalogPage({
      sessionDir: directory,
      offset: 0,
      limit: 10,
    });
    assert.deepEqual(
      page?.sessions.map((entry) => entry.id),
      ["large"],
    );
    assert.match(page?.sessions[0].allMessagesText || "", /newest/);

    const missingScope = await catalog.tryListSessionCatalogPage({
      sessionDir: directory,
      cwd: "/work/missing",
      offset: 0,
      limit: 2,
    });
    assert.deepEqual(missingScope?.sessions, []);
  });
});

test("session catalog rebuilds an empty directory and sync falls back to the session path", async () => {
  await withSessionDir(async (directory) => {
    assert.deepEqual(await catalog.rebuildSessionCatalog(directory), {
      checked: 0,
      indexed: 0,
    });
    const sessionFile = path.join(directory, "fallback.jsonl");
    await writeSession(
      sessionFile,
      sessionEntries("fallback", "", "2026-07-15T04:00:00.000Z"),
    );
    assert.equal(
      catalog.updateSessionCatalogFromSessionManagerSync({
        sessionFile,
        fileEntries: sessionEntries("fallback", "", "2026-07-15T04:00:00.000Z"),
      }),
      true,
    );
    await catalog.ensureSessionCatalog(directory);
    const page = await catalog.tryListSessionCatalogPage({
      sessionDir: directory,
      offset: 0,
      limit: 5,
    });
    assert.equal(page?.sessions[0].id, "fallback");
  });
});

test("session catalog rejects corrupt SQLite schema variants and rebuilds cleanly", async () => {
  const mutations = [
    "UPDATE catalog_meta SET value = '999' WHERE key = 'schema_version'",
    "DROP TABLE sessions",
    "DELETE FROM catalog_meta WHERE key = 'schema_version'",
    "DROP TABLE sessions; CREATE TABLE sessions(path TEXT PRIMARY KEY)",
    "DROP TABLE excluded_files; CREATE TABLE excluded_files(path TEXT PRIMARY KEY)",
  ];
  await withSessionDir(async (directory) => {
    const dbPath = path.join(
      directory,
      ".rin-session-catalog",
      "v3",
      "catalog.sqlite",
    );
    await fs.mkdir(path.dirname(dbPath), { recursive: true });
    new BetterSqlite3(dbPath).close();
    assert.equal(
      await catalog.tryListSessionCatalogPage({
        sessionDir: directory,
        offset: 0,
        limit: 5,
      }),
      undefined,
    );
    await fs.rm(dbPath);
    await fs.writeFile(dbPath, "not sqlite");
    assert.equal(
      await catalog.tryListSessionCatalogPage({
        sessionDir: directory,
        offset: 0,
        limit: 5,
      }),
      undefined,
    );
  });

  for (const mutation of mutations) {
    await withSessionDir(async (directory) => {
      await writeSession(
        path.join(directory, "valid.jsonl"),
        sessionEntries("valid", "/work/valid", "2026-07-15T04:00:00.000Z"),
      );
      await catalog.rebuildSessionCatalog(directory);
      const dbPath = path.join(
        directory,
        ".rin-session-catalog",
        "v3",
        "catalog.sqlite",
      );
      const db = new BetterSqlite3(dbPath);
      db.exec(mutation);
      db.close();
      assert.equal(
        await catalog.tryListSessionCatalogPage({
          sessionDir: directory,
          offset: 0,
          limit: 5,
        }),
        undefined,
      );
      await catalog.ensureSessionCatalog(directory);
      assert.deepEqual(
        (await catalog.listAllSessionCatalog({ sessionDir: directory }))?.map(
          (entry) => entry.id,
        ),
        ["valid"],
      );
    });
  }
});

test("session catalog normalizes sparse persisted row metadata", async () => {
  await withSessionDir(async (directory) => {
    await writeSession(
      path.join(directory, "row.jsonl"),
      sessionEntries("row", "/work/row", "2026-07-15T04:00:00.000Z"),
    );
    await catalog.rebuildSessionCatalog(directory);
    const db = new BetterSqlite3(
      path.join(directory, ".rin-session-catalog", "v3", "catalog.sqlite"),
    );
    db.prepare(
      `UPDATE sessions SET
         name = '', message_count = 0, cwd = '', parent_session_path = '',
         indexed_entry_count = 0, indexed_last_entry_signature = '',
         indexed_entries_hash = '', directory_ordinal = 0,
         file_size = 0, file_mtime_ms = 0, file_ctime_ms = 0`,
    ).run();
    db.close();
    const page = await catalog.tryListSessionCatalogPage({
      sessionDir: directory,
      offset: 0,
      limit: 5,
    });
    assert.equal(page?.sessions[0].name, undefined);
    assert.equal(page?.sessions[0].messageCount, 0);
    assert.equal(page?.sessions[0].cwd, undefined);
    assert.equal(page?.sessions[0].parentSessionPath, undefined);
  });
});

test("session catalog reconciles excluded, changed, added, and removed files", async () => {
  await withSessionDir(async (directory) => {
    const legacyV1 = path.join(directory, ".rin-session-catalog", "v1");
    const legacyV2 = path.join(directory, ".rin-session-catalog", "v2");
    await fs.mkdir(legacyV1, { recursive: true });
    await fs.mkdir(legacyV2, { recursive: true });
    const first = path.join(directory, "first.jsonl");
    await fs.writeFile(first, "not-json\n");
    assert.deepEqual(await catalog.rebuildSessionCatalog(directory), {
      checked: 1,
      indexed: 0,
    });
    await assert.rejects(fs.stat(legacyV1));
    await assert.rejects(fs.stat(legacyV2));
    assert.deepEqual(
      await catalog.listAllSessionCatalog({ sessionDir: directory }),
      [],
    );

    await catalog.ensureSessionCatalog(directory);
    await writeSession(
      first,
      sessionEntries("first", "/work/a", "2026-07-15T04:00:00.000Z"),
    );
    await catalog.ensureSessionCatalog(directory);
    const firstFreshness = await fs.stat(first);
    await new Promise((resolve) => setTimeout(resolve, 5));
    await writeSession(
      first,
      sessionEntries("other", "/work/a", "2026-07-15T04:00:00.000Z"),
    );
    await fs.utimes(first, firstFreshness.atime, firstFreshness.mtime);
    await catalog.ensureSessionCatalog(directory);
    const second = path.join(directory, "second.jsonl");
    await writeSession(
      second,
      sessionEntries("second", "/work/b", "2026-07-15T05:00:00.000Z"),
    );
    await catalog.ensureSessionCatalog(directory);
    assert.deepEqual(
      (await catalog.listAllSessionCatalog({ sessionDir: directory }))?.map(
        (entry) => entry.id,
      ),
      ["second", "other"],
    );
    assert.deepEqual(
      await catalog.listAllSessionCatalog({
        sessionDir: directory,
        cwd: "/work/missing",
      }),
      [],
    );

    await fs.rm(first);
    await catalog.ensureSessionCatalog(directory);
    assert.deepEqual(
      (await catalog.listAllSessionCatalog({ sessionDir: directory }))?.map(
        (entry) => entry.id,
      ),
      ["second"],
    );
  });
});

test("session manager sync catalog accepts persisted sessions and handles malformed managers", async () => {
  await withSessionDir(async (directory) => {
    assert.equal(
      catalog.updateSessionCatalogFromSessionManagerSync({
        isPersisted: () => false,
      }),
      false,
    );
    assert.equal(
      catalog.updateSessionCatalogFromSessionManagerSync({
        isPersisted: () => true,
        fileEntries: [],
      }),
      false,
    );
    assert.equal(
      catalog.updateSessionCatalogFromSessionManagerSync({
        isPersisted: () => true,
        sessionFile: directory,
        getSessionDir: () => directory,
      }),
      false,
    );
    assert.equal(
      catalog.updateSessionCatalogFromSessionManagerSync({
        isPersisted: () => true,
        sessionFile: path.join(directory, "missing-header.jsonl"),
        fileEntries: [{ type: "message" }],
      }),
      false,
    );
    const filePath = path.join(directory, "live.jsonl");
    await writeSession(
      filePath,
      sessionEntries("live", "/work/live", "2026-07-15T04:00:00.000Z"),
    );
    assert.equal(
      catalog.updateSessionCatalogFromSessionManagerSync({
        isPersisted: () => true,
        sessionFile: filePath,
        fileEntries: sessionEntries(
          "live",
          "/work/live",
          "2026-07-15T04:00:00.000Z",
        ),
        getSessionDir: () => directory,
      }),
      true,
    );
    const childPath = path.join(directory, "child.jsonl");
    const childEntries = sessionEntries(
      "child",
      "/work/live",
      "2026-07-15T05:00:00.000Z",
    );
    await writeSession(childPath, childEntries);
    assert.equal(
      catalog.updateSessionCatalogFromSessionManagerSync({
        isPersisted: () => true,
        sessionFile: childPath,
        fileEntries: childEntries,
        getSessionDir: () => directory,
      }),
      true,
    );
    assert.equal(
      catalog.updateSessionCatalogFromSessionManagerSync({
        isPersisted: () => true,
        sessionFile: filePath,
        getSessionDir: () => path.join(directory, "other"),
      }),
      false,
    );
    assert.equal(
      catalog.updateSessionCatalogFromSessionManagerSync({
        isPersisted: () => true,
        sessionFile: filePath,
        getSessionDir: () => {
          throw new Error("broken session dir");
        },
      }),
      false,
    );
    assert.equal(
      catalog.updateSessionCatalogFromSessionManagerSync({
        isPersisted: () => {
          throw new Error("broken manager");
        },
      }),
      false,
    );
    await catalog.ensureSessionCatalog(directory);
    const page = await catalog.tryListSessionCatalogPage({
      sessionDir: directory,
      offset: 0,
      limit: 0,
    });
    assert.deepEqual(page?.sessions, []);
  });
});
