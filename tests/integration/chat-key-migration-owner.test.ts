...
test("chat key migration handles adapter variants and incomplete credentials", () => {
  assert.deepEqual(migration.inferChatBotIdsFromSettings(null), {});
  assert.deepEqual(
    migration.inferChatBotIdsFromSettings({
      chat: {
        byChatKey: { ignored: true },
        turnPolicy: "queue",
        quietMode: true,
        telegram: [{ selfId: " tg-explicit " }, null, "invalid"],
        discord: {
          primary: { clientId: "discord-client" },
          invalid: { token: "not-a-snowflake.payload" },
        },
        qq: { id: "qq-id" },
        lark: { appId: "lark-app" },
        slack: { botUserId: "slack-bot" },
        custom: { id: "custom-id" },
        empty: {},
        invalid: "text",
      },
    }),
    {
      ...
      qq: "qq-id",
      slack: "slack-bot",
      telegram: "tg-explicit",
    },
  );
});

test("chat key migration exercises empty, nested, and explicit adapter identities", () => {
  assert.deepEqual(
    migration.inferChatBotIdsFromSettings({
      chat: {
    ...
    }),
    {
      ...
      qq: "qq-bot",
      slack: "slack-user",
      telegram: "telegram-explicit",
    },
  );
});

test("chat key migration rejects malformed key shapes", () => {
  assert.equal(migration.parseLegacyUnqualifiedChatKey("no-separator"), null);
  assert.equal(migration.parseLegacyUnqualifiedChatKey(" :chat"), null);
  assert.equal(migration.canonicalizeStoredChatKey("", {}), "");
  assert.equal(migration.canonicalizeStoredChatKey("telegram/:chat", {}), "");
  assert.equal(migration.canonicalizeStoredChatKey("telegram:chat", {}), "");
});

test("chat key migration reports unresolved and conflicting settings keys", () => {
  assert.deepEqual(migration.rewriteSettingsChatKeys(null), {
    settings: {},
    rewritten: {},
    unresolved: [],
    conflicts: {},
  });
  assert.deepEqual(migration.rewriteSettingsChatKeys({ chat: true }), {
    settings: { chat: true },
    rewritten: {},
    unresolved: [],
    conflicts: {},
  ...
  assert.deepEqual(
    migration.rewriteSettingsChatKeys({ chat: { byChatKey: [] } }),
    {
      settings: { chat: { byChatKey: [] } },
      rewritten: {},
      unresolved: [],
      conflicts: {},
    },
  ...
  const result = migration.rewriteSettingsChatKeys(
    {
      chat: {
        byChatKey: {
          "telegram:chat": { legacy: true, shared: "legacy" },
          "telegram/bot:chat": { canonical: true, shared: "canonical" },
          unresolved: "kept",
        },
      },
    },
  ...
  assert.deepEqual(result.rewritten, {
    "telegram:chat": "telegram/bot:chat",
  });
  assert.deepEqual(result.unresolved, ["unresolved"]);
  assert.deepEqual(result.conflicts, {
    "telegram/bot:chat": ["telegram/bot:chat"],
  });
...
test("chat key migration keeps canonical conflict precedence in either source order", () => {
  const canonicalFirst = migration.rewriteSettingsChatKeys(
    {
      chat: {
        byChatKey: {
          "telegram/bot:chat": { shared: "canonical", canonical: true },
          "telegram:chat": { shared: "legacy", legacy: true },
          " telegram:chat ": "second legacy",
        },
      },
    },
    { telegram: "bot" },
  );
  assert.deepEqual(canonicalFirst.settings.chat.byChatKey, {
    "telegram/bot:chat": {
      shared: "canonical",
      canonical: true,
      legacy: true,
  ...
  assert.deepEqual(canonicalFirst.conflicts, {
    "telegram/bot:chat": ["telegram:chat", " telegram:chat "],
  });
});
  ...
  try {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        ...
        strippedContent: "current stripped content",
        elements: [{ type: "text", attrs: { content: "current" } }],
    ...
    const fallbackCanonicalKey = createHash("sha1")
      .update(`lark/cli_bot:oc_same\n${fallbackMessageId}`)
      .digest("hex");
    const fallbackCanonicalPath = path.join(
      recordsDir,
      fallbackCanonicalKey.slice(0, 2),
      `${fallbackCanonicalKey}.json`,
    );
    await fs.mkdir(path.dirname(fallbackCanonicalPath), { recursive: true });
    await fs.writeFile(
      fallbackCanonicalPath,
      JSON.stringify({
        version: 1,
        recordKey: fallbackCanonicalKey,
    ...
    const fallbackLegacy = await writeLegacyRecord(fallbackMessageId, {
      role: "assistant",
      acceptedAt: "2026-06-01T00:00:01.000Z",
      processedAt: "2026-06-01T00:00:02.000Z",
      sessionFile: "sessions/fallback.jsonl",
      lastReceivedAt: "2026-06-01T00:00:03.000Z",
    ...
    const duplicateLegacy = await writeLegacyRecord("duplicate-message", {
      ...
      elements: [
        { type: "text", attrs: { content: "legacy one" } },
        { type: "text", attrs: { content: "legacy two" } },
      ],
    ...
    const malformedDir = path.join(recordsDir, "malformed");
    await fs.mkdir(malformedDir, { recursive: true });
    await fs.writeFile(path.join(malformedDir, "ignored.txt"), "ignored");
    await fs.writeFile(path.join(malformedDir, "invalid.json"), "not json");
    await fs.writeFile(
      path.join(malformedDir, "missing-key.json"),
      JSON.stringify({ messageId: "missing-key" }),
    );
    await fs.writeFile(
      path.join(malformedDir, "missing-message.json"),
      JSON.stringify({ recordKey: "missing-message" }),
    );
    assert.equal(
    ...
    assert.equal(result.migratedRecords, 3);
    ...
    await assert.rejects(fs.access(fallbackLegacy.filePath));
