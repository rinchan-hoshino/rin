...
test(
  ...
  async () => {
    ...
    const ownerScenario = (globalThis as any).__rinUpdateWorkflowScenario;
    if (ownerScenario) ownerScenario.mockManagedNpmDownload = true;
    try {
      ...
      if (ownerScenario) ownerScenario.mockManagedNpmDownload = false;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  },
);
