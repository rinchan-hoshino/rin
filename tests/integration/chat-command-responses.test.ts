...
test("chat command responses normalize missing and blank configuration", () => {
  assert.equal(
    commandResponses.resolveChatCommandResponses(undefined).new,
    "Started a new session.",
  );
  assert.equal(
    commandResponses.resolveChatCommandResponses({ new: "   " }).new,
    "Started a new session.",
  );
  assert.equal(
    commandResponses.readChatCommandResponses("").new,
    "Started a new session.",
  );
});
