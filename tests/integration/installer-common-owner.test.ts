...
test("installer command helpers select shell and invoking user semantics", () => {
  ...
  assert.equal(common.shouldRunCommandThroughShell("node"), false);
  assert.equal(
    common.detectExecutorUser({ osUser: "  deployment-owner  " }),
    "deployment-owner",
  );
  assert.equal(common.detectExecutorUser({ osUser: "" }), "unknown");
  ...
  try {
    assert.equal(common.detectExecutorUser(), "unknown");
  } finally {
    (os as any).userInfo = originalUserInfo;
  }
