...
test("chat outbox normalizes payloads and enforces idempotency keys", async () => {
  assert.equal(outbox.normalizeChatOutboxPayload(null), null);
  assert.equal(
    outbox.normalizeChatOutboxPayload({ chatKey: "x", parts: [] }),
    null,
  );
  assert.equal(
  ...
  const normalized = outbox.normalizeChatOutboxPayload({
    chatKey: " telegram/1:2 ",
    parts: [null, { type: "text", text: "hello" }],
  });
  assert.ok(normalized);
  assert.equal(normalized.chatKey, "telegram/1:2");
  assert.deepEqual(normalized.parts, [{ type: "text", text: "hello" }]);
  ...
  await withTempDir(async (dir) => {
    ...
    const firstPath = outbox.enqueueChatOutboxPayload(dir, payload, {
      idempotencyKey: "same-delivery",
    });
    assert.equal(
      outbox.enqueueChatOutboxPayload(dir, payload, {
        idempotencyKey: "same-delivery",
      }),
      firstPath,
    );
    const stored = outbox.readChatOutboxItem(dir, firstPath);
    stored.idempotencyKey = "different-delivery";
    outbox.writeChatOutboxItem(dir, stored);
    assert.throws(
      () =>
        outbox.enqueueChatOutboxPayload(dir, payload, {
          idempotencyKey: "same-delivery",
