...
test("terminal outbox commit owns the inbound turn before external delivery", async () => {
  await withTempDir(async (dir) => {
    ...
    assert.equal(outbox.getActiveChatOutboxTurnFence(), undefined);
    outbox.runWithChatOutboxTurnFence(turnFence, () => {
      assert.deepEqual(outbox.getActiveChatOutboxTurnFence(), turnFence);
    });
    assert.equal(outbox.isChatOutboxTurnFenceActive(dir, turnFence), true);
    assert.equal(
      outbox.isChatOutboxTurnFenceActive(dir, turnFence, "telegram/other"),
      false,
    );
    assert.equal(
      outbox.isChatOutboxTurnFenceActive(dir, {
        ...turnFence,
        agentDir: `${dir}-other`,
      }),
      false,
    ...
    assert.equal(
      outbox.hasCommittedTerminalChatOutbox(
        dir,
        claim.chatKey,
        claim.messageId,
      ),
    ...
    assert.equal(outbox.isCommittedTerminalChatOutboxItem(dir, id), true);
    assert.equal(
      outbox.isCommittedTerminalChatOutboxItem(dir, "missing-outbox"),
      false,
    );
    assert.deepEqual(
      outbox.listCommittedChatOutboxItems(dir).map((item) => item.id),
      [id],
    );
    assert.deepEqual(outbox.listChatOutboxHistoryItems(dir, "delivered"), []);
    assert.equal(outbox.markChatOutboxPostDeliveryApplied(dir, id), true);
    assert.equal(outbox.markChatOutboxPostDeliveryApplied(dir, id), false);
    assert.deepEqual(outbox.listCommittedChatOutboxItems(dir), []);
    assert.equal(
...
test("authoritative terminal turns reject mismatches and accept current ownership", async () => {
  await withTempDir(async (dir) => {
    const inbound = inbox.enqueueChatInboxItem(dir, {
      chatKey: "telegram/777:terminal-authority",
      messageId: "terminal-authority-message",
      session: {
        platform: "telegram",
        selfId: "777",
        channelId: "terminal-authority",
        userId: "owner",
        messageId: "terminal-authority-message",
        timestamp: Date.now(),
        content: "terminal authority",
        stripped: { content: "terminal authority" },
      },
      elements: [{ type: "text", attrs: { content: "terminal authority" } }],
    }).item;
    const claim = inbox.claimChatInboxItem(dir, inbound.itemId);
    const terminalTurn = {
      turnId: claim.itemId,
    ...
    const terminalOptions = (recordId: string) => ({
      deliveryKind: "final" as const,
      terminalRecordId: recordId,
      id: `chat-${recordId}`,
      idempotencyKey: `chat-${recordId}`,
    });
    ...
    assert.throws(
      () =>
        outbox.enqueueChatOutboxPayload(
          dir,
          { ...payload("mismatched chat"), chatKey: claim.chatKey },
          {
            ...terminalOptions("terminal-chat-mismatch"),
            terminalTurn: { ...terminalTurn, chatKey: "telegram/other" },
          },
        ),
    ...
    assert.throws(
      () =>
        outbox.enqueueChatOutboxPayload(
          dir,
          { ...payload("missing turn"), chatKey: claim.chatKey },
          {
            ...terminalOptions("terminal-missing-turn"),
            terminalTurn: {
              ...terminalTurn,
              messageId: "missing-terminal-message",
            },
          },
        ),
      /chat_terminal_turn_mismatch/,
    ...
    const id = outbox.enqueueChatOutboxPayload(
      dir,
      { ...payload("authoritative"), chatKey: claim.chatKey },
      {
        ...terminalOptions("terminal-authoritative"),
        terminalTurn,
      },
    );
    assert.equal(id, "chat-terminal-authoritative");
    assert.equal(
      outbox.readChatOutboxItemById(dir, id)?.item.turnId,
      claim.itemId,
    );
    assert.equal(inbox.getChatInboxItem(dir, claim.itemId)?.state, "terminal");
  });
});
...
test("chat generation interrupts queued nonterminal outbox work without suppressing committed finals", async () => {
  await withTempDir(async (dir) => {
    ...
    assert.equal(staleInterim.lastError, "chat_outbox_turn_interrupted");
