...
test("rpc runtime owns prompt admission, queue visibility, and recovery without losing turns", async () => {
  ...
  assert.equal(sentOf(client, "steer").at(-1)?.message, "owner steer");
  assert.equal(sentOf(client, "follow_up").at(-1)?.requestTag, "follow");
  await session.resumeInterruptedTurn({ source: "restore" });
  assert.equal(
    sentOf(client, "resume_interrupted_turn").at(-1)?.source,
    "restore",
  );
  ...
  assert.deepEqual(session.getSteeringMessages(), ["queued steer"]);
  assert.deepEqual(session.getFollowUpMessages(), ["queued follow"]);
  ...
  assert.deepEqual(session.getSteeringMessages(), ["retry me"]);
  ...
  await session.prompt("timeout me");
  assert.equal(session.timedOutPromptOps.length, 1);
  assert.equal(session.recoveryPending, true);
  assert.equal(reconnectCalls >= 3, true);
...
test("rpc runtime connection events expose deterministic frontend phases and cleanup", async () => {
  ...
  session.setRemoteTurnRunning(false);
