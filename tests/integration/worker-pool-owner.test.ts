...
test("worker state and terminal APIs preserve explicit error and fallback contracts", async () => {
  const dir = await makeTempDir("rin-worker-pool-errors-");
  const workerPath = path.join(dir, "worker-source");
  const sessionFile = path.join(dir, "session.jsonl");
  await fs.writeFile(
    workerPath,
    String.raw`let count = 0;
process.stdin.setEncoding('utf8');
let buffer = '';
process.stdin.on('data', (chunk) => {
  buffer += chunk;
  while (buffer.includes('\n')) {
    ...
    if (command.type === 'get_state') {
      count += 1;
      process.stdout.write(JSON.stringify({ id: command.id, type: 'response', command: command.type, success: count > 1 }) + '\n');
      continue;
    }
    process.stdout.write(JSON.stringify({ id: command.id, type: 'response', command: command.type, success: true, data: {} }) + '\n');
    if (command.type === 'resume_interrupted_turn') {
      process.stdout.write(JSON.stringify({
        type: 'rpc_turn_event',
        event: command.requestTag === 'fallback-tag' ? 'complete' : 'error',
        requestTag: command.requestTag,
        error: '',
...
`,
  ...
  await assert.rejects(
    pool.readWorkerState(worker),
    /rin_worker_state_unavailable/,
  );
  assert.deepEqual(await pool.readWorkerState(worker), {});
  await assert.rejects(
    pool.resumeInterruptedTurnSession({}),
    /rin_session_file_required/,
  );
  await assert.rejects(
    pool.resumeInterruptedTurnSession({
      sessionFile,
      requestTag: "error-tag",
    }),
    (error: any) => {
      assert.equal(error.message, "rin_turn_failed");
      assert.equal(error.sessionId, "terminal-error");
      assert.equal(error.sessionFile, sessionFile);
      assert.equal(error.rinTurnTerminal, true);
      return true;
    },
  );
  assert.deepEqual(
    await pool.resumeInterruptedTurnSession({
      sessionFile,
      requestTag: "fallback-tag",
    }),
    {
      finalText: "",
      result: undefined,
