...
test("current-user preparation can load the configured Pi tools manager", async () => {
  const moduleUrl =
    "data:text/javascript," +
    encodeURIComponent(
      "export async function ensureTool(tool) { return `/managed/${tool}`; }",
    );
  const result = await piTools.preparePiManagedToolsForInstall(localOptions, {
    env: {},
    toolsManagerModuleUrl: moduleUrl,
  });
...
test("current-user preparation reports a manager without ensureTool", async () => {
  const warnings: string[] = [];
  const moduleUrl = `data:text/javascript,${encodeURIComponent("export const version = 1;")}`;
  const result = await piTools.preparePiManagedToolsForInstall(localOptions, {
    env: {},
    toolsManagerModuleUrl: moduleUrl,
  ...
  assert.match(warnings[0], /rin_installer_fd_manager_unavailable/);
});
