...
await import("../support/register-capability-session-owner-fixture.js");
...
test("unbound capability contexts expose exact defensive defaults", async () => {
  ...
  const originalDefaultContext = set.createContext();
  assert.equal(originalDefaultContext.getThinkingLevel(), "medium");
  originalDefaultContext.emitEvent({ type: "ignored-before-bind" });

  // The owner loader exposes this otherwise-private defensive table only in
  // this process, allowing every exact no-op contract to remain executable.
  const defaultCore = (capabilitySession as any).__rinOwnerNoOpCoreActions;
  defaultCore.sendMessage();
  defaultCore.sendUserMessage();
  defaultCore.appendEntry();
  defaultCore.setSessionName();
  assert.equal(defaultCore.getSessionName(), undefined);
  defaultCore.setLabel();
  assert.deepEqual(defaultCore.getActiveTools(), []);
  assert.deepEqual(defaultCore.getAllTools(), []);
  defaultCore.setActiveTools();
  defaultCore.refreshTools();
  assert.equal(await defaultCore.setModel(), false);
  defaultCore.setThinkingLevel();
