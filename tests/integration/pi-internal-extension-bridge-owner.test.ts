...
test("capability bridge observes agent settlement without blocking or leaking failures", async () => {
  const runner: any = {
    hasHandlers: () => false,
    emit() {
      throw new Error("extension observer failed");
    },
  };
  const session = { extensionRunner: runner };
  const capabilitySet = {
    hasHandlers: (name: string) => name === "agent_settled",
    async emit() {
      throw new Error("capability observer failed");
    },
  };
  const errors: string[] = [];
  ...
  try {
    ...
    assert.match(errors.join("\n"), /extension observer failed/);
    assert.match(errors.join("\n"), /capability observer failed/);

    errors.length = 0;
    bridge.attachRinCapabilityExtensionBridge(session, {
      hasHandlers: () => false,
      async emit() {
        assert.fail("unexpected capability observer");
      },
    });
    assert.equal(await runner.emit({ type: "agent_settled" }), undefined);
    await new Promise((resolve) => setImmediate(resolve));
