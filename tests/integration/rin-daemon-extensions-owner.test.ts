...
function manager(cwd: string, agentDir: string, warnings: string[]) {
  return new extensions.RinDaemonExtensionManager({
    cwd,
    agentDir,
    logger: {
...
test("daemon dependency ownership writes exact runtime package and installs only when stale", async () => {
  reset();
  await withSandbox(async ({ agentDir }) => {
    const emptyRoot = extensions.ensureDaemonExtensionDependencies(agentDir, [
    ...
    const runtimeRoot = extensions.ensureDaemonExtensionDependencies(agentDir, [
    ...
    extensions.ensureDaemonExtensionDependencies(agentDir, [
      packageEntry("z-owner", { version: "2" }),
      packageEntry("a-owner"),
    ]);
    assert.equal(events.filter(([name]) => name === "exec").length, 1);
...
test("daemon manager owns extension API, service lifecycle, adapters, and memory providers", async () => {
  reset();
...
test("daemon discovery deduplicates explicit, Pi, and cwd extensions and isolates failures", async () => {
  reset();
  await withSandbox(async ({ cwd, agentDir }) => {
    ...
    await fs.writeFile(
      modulePath,
      `export function rinDaemonExtension(api) { api.registerBackgroundService({ start() {} }); }\n`,
    );
    ...
    assert.equal(
      warnings.some((message) =>
        message.includes("daemon extension init failed"),
      ),
      true,
    );
...
test("daemon module entrypoint shapes and stop failures remain isolated", async () => {
  for (const [name, moduleValue, expected] of [
    [
      "create-service",
      {
        rinDaemonExtension(api: any) {
          api.registerBackgroundService({ start() {} });
        },
      },
      true,
    ],
    ["empty-extension", { rinDaemonExtension() {} }, false],
    [
      "provider-service",
      {
        rinDaemonExtension(api: any) {
          api.registerBackgroundService(async () => ({ stop() {} }));
        },
      },
      true,
    ],
  ...
  await withSandbox(async ({ cwd, agentDir }) => {
    ...
    modules["handled-empty"] = { rinDaemonExtension() {} };
