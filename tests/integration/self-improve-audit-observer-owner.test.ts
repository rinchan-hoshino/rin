...
test("audit policy validation and snapshot bounds cover malformed and binary evidence", async () => {
  const agentDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "rin-self-improve-audit-bounds-"),
  );
  try {
    assert.deepEqual(sanitizeSelfImproveHistoryText("", 8), {
      text: "",
      redacted: false,
      truncated: false,
    });
    assert.equal(
      sanitizeSelfImproveHistoryText('Authorization: "owner-secret"', 128)
        .redacted,
      true,
    );
    assert.equal(
      sanitizeSelfImproveHistoryText("sk-12345678901234567890", 128).redacted,
      true,
    );
    ...
    await assert.rejects(
      () =>
        beginSelfImproveRunAudit({
          agentDir,
          runId: "non-finite-policy",
          kind: "review",
          startedAt: "2026-08-02T00:00:00.000Z",
          policy: { maxOutputBytes: Number.NaN },
        }),
      /self_improve_audit_invalid_policy:maxOutputBytes/,
    );
    await assert.rejects(
    ...
    for (const mode of ["root", "entry"] as const) {
      const symlinkAgent = fs.mkdtempSync(
        path.join(os.tmpdir(), `rin-self-improve-audit-symlink-${mode}-`),
      );
      try {
        const prompts = path.join(symlinkAgent, "self_improve", "prompts");
        fs.mkdirSync(path.dirname(prompts), { recursive: true });
        if (mode === "root") {
          fs.symlinkSync(os.tmpdir(), prompts);
        } else {
          fs.mkdirSync(prompts, { recursive: true });
          fs.symlinkSync(os.tmpdir(), path.join(prompts, "unsafe"));
        }
        await assert.rejects(
          () =>
            beginSelfImproveRunAudit({
        ...
        fs.rmSync(symlinkAgent, { recursive: true, force: true });
      }
    }

    assert.equal(
      await resolveSafeSelfImprovePath(agentDir, agentDir),
      agentDir,
    );
    await assert.rejects(
    ...
    const symlinkPath = path.join(agentDir, "unsafe-link");
    fs.symlinkSync(os.tmpdir(), symlinkPath);
    await assert.rejects(
      () => resolveSafeSelfImprovePath(agentDir, path.join(symlinkPath, "x")),
      /self_improve_audit_symlink_path/,
    );

    const promptsDir = path.join(agentDir, "self_improve", "prompts");
    const skillsDir = path.join(agentDir, "self_improve", "skills", "binary");
    fs.mkdirSync(promptsDir, { recursive: true });
    fs.mkdirSync(skillsDir, { recursive: true });
    fs.writeFileSync(
      path.join(promptsDir, "large.md"),
      "secret=" + "x".repeat(64),
    );
    fs.writeFileSync(path.join(skillsDir, "nul.bin"), Buffer.from([0, 1, 2]));
    fs.writeFileSync(
      path.join(skillsDir, "invalid.bin"),
    ...
    const handle = await beginSelfImproveRunAudit({
      ...
      source: {
        sessionFile: "",
        leafId: "secret=owner-source-secret",
        snapshotKey: "snapshot-" + "x".repeat(32),
        trigger: undefined,
      },
      policy: {
        maxMetadataBytes: 8,
        maxSnapshotFileBytes: 2,
        maxSnapshotBytes: 2,
        maxOutputBytes: 8,
        maxErrorBytes: 8,
        maxPatchBytes: 8,
    ...
    fs.writeFileSync(path.join(promptsDir, "large.md"), "changed");
    const reference = await completeSelfImproveRunAudit({
      agentDir,
      handle,
      status: "skipped",
      ...
      error: "password=abcdefghijklmnopqrstuvwxyz",
    });
    assert.equal(reference.complete, false);
    assert.equal(reference.truncated, true);
    assert.equal(reference.redacted, true);
