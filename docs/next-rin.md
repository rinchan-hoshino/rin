...
Codex uses app-server for input and paginated history observation; Claude Code, pi, and OpenCode use their documented non-interactive session commands. Before a Codex-backed chat service starts, Rin runs the configured CLI's idempotent `codex app-server daemon start` command and requires it to succeed. It does not classify how Codex was installed. The guided installation identifies its current agent and enables a fresh conversation for each admitted chat. Each adapter creates and continues native sessions and reports public answer output.
