...
The setup asks whether to replace an old Pi-based Rin, which products to install (Codex CLI, ChatGPT desktop app, both, or neither), whether the user wants a future reviewed recommendation profile, and what initial global AGENTS instructions to use. An existing AGENTS file is preserved unless the user chooses to append instructions. There is no shipped recommendation profile yet: answering yes records that preference but does not change model, sandbox, approvals, or private connector settings.
