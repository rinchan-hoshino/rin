...
Rin owns one Hermes-derived continuation-checkpoint pipeline while reusing Pi's model, authentication, retry, preparation, file-detail, and recent-tail machinery. Pi's native summary prompt is not prepended. Before an 85% threshold can compact, provider-bound tool-result retention ratchets from four 16-call buckets down to one bucket and re-estimates context after each step; compaction runs only when one retained bucket still exceeds the threshold. Recent source retained verbatim targets 20% of the effective compaction threshold rather than a fixed token count. If Pi's token cut lands inside a turn, Rin moves the cut back far enough to keep the latest real user message and latest visible assistant reply in their original roles, so an active request is never available only through the reference summary and the in-flight tool chain remains intact. The summarizer uses the Hermes checkpoint wording with Rin's deterministic pruned-skill reload marker, an explicit previous-summary/new-turn split, and a 160,000-character bounded input. Its target budget is 20% of compacted source within 2,000–10,000 tokens and is guidance rather than a hard output cap. Every summary is wrapped in the Hermes reference-only boundary. A reload required after compaction is deferred until the active agent turn ends, so current extension tool contexts remain valid. Todo remains tool-owned branch state and is not copied into the checkpoint.
