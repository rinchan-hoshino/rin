...
- During daemon restart, the core socket becomes callable before transcript-index and durable-turn recovery finish. `daemon_status` reports `startup.phase = recovering_index | recovering_turns | ready`; every other RPC remains fenced with `rin_daemon_recovering` until recovery reaches `ready`. Transcript repair and durable-turn recovery stay serial, and the local Chat connector plus hosted adapters start only after that fence opens. This lets updater readiness prove the new core process without allowing Chat adapters to claim work ahead of recovery.
...
Rin owns one Hermes-derived continuation-checkpoint pipeline while reusing Pi's model, authentication, retry, preparation, file-detail, and recent-tail machinery. Pi's native summary prompt is not prepended. The 85% threshold is evaluated after ordinary fixed four-bucket provider pruning; once reached, compaction runs without a separate pressure-pruning stage. Recent source retained verbatim targets 20% of the effective compaction threshold rather than a fixed token count. If Pi's token cut lands inside a turn, Rin follows Hermes' task-anchor rule and moves the cut back far enough to keep the latest real user message and latest visible assistant reply in their original roles, so an active request is never available only through the reference summary and the in-flight tool chain remains intact. After a compaction entry exists, the provider-bound tail follows Hermes lean retention: the newest eight assistant tool-call rounds remain verbatim while eligible older tool-result bodies use Rin's existing omission form; the append-only session entries remain unchanged. The lean step does not rewrite append-only session entries or the checkpoint already generated from the pre-compaction input, but repeated provider calls no longer resend every bulky result retained behind the user anchor. The summarizer uses the Hermes checkpoint wording with Rin's deterministic pruned-skill reload marker, an explicit previous-summary/new-turn split, and a 160,000-character bounded input. Its target budget is 20% of compacted source within 2,000–10,000 tokens and is guidance rather than a hard output cap. Every summary is wrapped in the Hermes reference-only boundary. After successful compaction, Pi has already rebuilt the compacted agent messages, so Rin refreshes only the durable system prompt in place; full session reload remains explicit, and active-turn steer input is not blocked behind `agent_end`. After 25 hours without session activity, the next prompt attempts one compaction and continues regardless of the result. Todo remains tool-owned branch state and is not copied into the checkpoint.
