...
- Periodic memory review runs every `selfImprove.reviewEveryTurns` real agent final messages and defaults to `8`; review also runs before compaction. Session shutdown updates session summaries but does not trigger an extra self-improve review.
