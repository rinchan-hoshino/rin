...
Rin owns one complete Hermes-style continuation-checkpoint prompt while reusing Pi's model, authentication, retry, preparation, and file-detail pipeline. Pi's native summary prompt is not prepended. Recent source retained verbatim is 20% of the effective compaction threshold rather than a fixed token count. The checkpoint preserves the historical task snapshot, goal, active constraints, concrete completed actions, live state, blockers, decisions, errors and fixes, resolved questions, relevant files, and critical operational details. Its target budget is 20% of the compacted source within 2,000–10,000 tokens and is guidance rather than a hard output cap. Each update receives the complete prior checkpoint, preserves everything still relevant, and removes only clearly obsolete state. When Pi selects a cut inside one tool-using turn, Rin summarizes the older history and the cut turn prefix together in one chronological pass. Todo remains tool-owned branch state and is not copied into the checkpoint.
