...
Rin owns one Hermes-derived continuation-checkpoint pipeline while reusing Pi's model, authentication, retry, preparation, file-detail, and recent-tail machinery. Pi's native summary prompt is not prepended. Recent source retained verbatim is 20% of the effective compaction threshold rather than a fixed token count. The summarizer receives an explicit previous-checkpoint/new-turn split, a 160,000-character middle-truncation bound that preserves both edges, and a structured template for the historical task snapshot, goal, active constraints, concrete completed actions, live state, blockers, decisions, errors and fixes, resolved questions, relevant files, critical details, and pruned-skill reload markers. The runtime wraps every checkpoint in a deterministic reference-only boundary: only a later real user message activates work, while current system/developer instructions and live producers remain authoritative. Its target budget is 20% of compacted source within 2,000–10,000 tokens and is guidance rather than a hard output cap. Each update preserves still-relevant prior state and applies later corrections chronologically. When Pi cuts inside one tool-using turn, Rin summarizes the older history and cut-turn prefix together in one pass. Todo remains tool-owned branch state and is not copied into the checkpoint.
