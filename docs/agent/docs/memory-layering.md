...
Rin runs memory review during fixed-round periodic maintenance, before session compaction, and on real session shutdown. Idle daemon worker sleep is not session shutdown and does not trigger shutdown memory maintenance. The periodic threshold is counted from real agent final messages, is configurable with `settings.json -> selfImprove.reviewEveryTurns`, and defaults to `8`; user steering inputs and assistant tool-call-only/interim messages do not count.
