...
Use this when a Rin chat reply needs native platform objects instead of plain text.
...
| Video          | `[video: name](url-or-local-path)`   | Use only when the target chat can receive video.                       |
| Audio          | `[audio: name](url-or-local-path)`   | Use only when the target chat can receive audio.                       |
| Sticker        | `[sticker: name](url-or-local-path)` | Use only when the target chat can receive stickers.                    |

## Practical rules

- Use native mentions only when the exact platform user id is known.
- Use quote replies only with the exact platform message id from the current chat context or message store.
