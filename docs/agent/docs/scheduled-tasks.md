...
- Use the `task_control` tool only to pause or resume an existing task when the task id is known.
- Use `rin status` or `rin status --json` for a redacted activity overview.
- Use daemon RPC for create, inspect, update, complete, or delete operations.
...
Use `cron_upsert_task` with the same `task.id`. Include the fields you intend to change; omitted fields reuse the existing task values.
