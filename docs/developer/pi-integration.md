...
- When adding a new Pi private seam, add a semantic helper in `src/core/pi/`,
  document it below, and update the guard test in `tests/architecture/pi-dependencies.test.ts`.
...
| Pi session/manager private methods and fields                                | `src/core/pi/session-host.ts`               | Rin uses public `resourceLoader`/`extensionRunner` getters when available, then centralizes remaining compaction, persistence, tool refresh, and event seams | `Pi session private members stay behind Rin's session host` |
| Rin-owned system prompt from Pi structured capability data                   | `src/core/rin-lib/system-prompt-overlay.ts` | `session-host.ts` binds the core prompt owner through the single private session seam; the prompt is not an extension resource                               | `Rin core owners stay outside Pi's extension registry`      |
| Rin context and compaction ownership                                         | `src/core/pi/session-host.ts`               | Context transforms compose after Pi's public agent transform; compaction uses semantic private session helpers while preserving user-extension lifecycle     | `Pi session private members stay behind Rin's session host` |
