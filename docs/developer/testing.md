...
Rin uses separate permanent test layers for separate failure signals. A test belongs to exactly one layer; its directory declares what a failure means.
...
Support code belongs in `tests/support` and is not a test layer.
