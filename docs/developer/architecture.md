...
- Process exit is not lifecycle input. Worker disappearance never terminalizes an active ledger row, and Chat/daemon shutdown flags only stop local admission or replacement work. Graceful shutdown may accept a terminal already being committed, but the shutdown reason itself writes no lifecycle transition. Work that outlives a process remains active and follows the same level-triggered reconciliation path used after `kill -9`.
