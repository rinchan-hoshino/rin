...
- The daemon admits each request into `data/core/daemon/turn-ledger.sqlite` before writing the command to a worker. The same immutable row freezes the runtime invocation provenance (`source`, frontend identity, and prompt context) so a replacement worker restores the original event contract before continuing; legacy active rows without that envelope remain unclassified rather than inferred. SQLite's native WAL mode provides transaction durability; Rin does not maintain an application-level terminal WAL.
...
- Chat owns `inbox_jobs`, messages, and platform outbox delivery only. It reconciles inherited running jobs before Chat platforms start and accepted jobs continuously afterward, attaching them to the daemon request without submitting the prompt again. A claim lease coordinates execution only: a reconciler atomically replaces the owner epoch before taking over, so stale executors lose their SQLite fence; process death or lease expiry never becomes a business terminal. Every interactive frontend submits the same ordinary prompt RPC. Rin serializes entry only until a causally attributed Pi-native event barrier is observed; that mutex is synchronization, never classification. An observed `agent_start`/owned user start yields `terminalOwner`, `queue_update` yields `nonterminal`, native rejection yields `rejected`, missing durable evidence yields `indeterminate`, and a duplicate stable request returns `rejoined(originalOutcome)`. Pi does not return a structured admission kind, and Rin never reconstructs one from `isStreaming`, requested streaming behavior, Working, a ledger row, or a controller. Before reporting a recoverable observed outcome, Rin appends a `rin_request_identity` custom entry to Pi JSONL linking the stable request ID, the exact user message entry ID, and its observed role. `rejected` and `indeterminate` results receive a durable `rin_request_outcome` receipt before response, so response-loss retries rejoin rather than resubmit. Identity receipts are accepted only when exactly one receipt follows exactly one referenced user entry; duplicate, conflicting, or out-of-order receipts remain `indeterminate` and are never replayed. `nonterminal` input never creates or transfers terminal ownership: the original `terminalOwner` still owns the single Pi terminal and final outbox. Ambiguous crash windows are never replayed automatically. Chat does not revive, supersede, infer, or mirror Pi lifecycle state.
- At Chat startup, inherited running jobs are ordered by estimated session-load memory and preconnected through a `MemAvailable` admission budget. Admission ends as soon as the selected session is open, before Pi-native continuation, so restart hydration peaks are staggered without limiting ordinary turn concurrency.
...
- Process exit is not lifecycle input. Worker disappearance never terminalizes an active ledger row, and Chat/daemon shutdown flags only stop local admission or replacement work. Daemon shutdown stops workers immediately and gives only local service teardown a short fixed deadline; it may accept a terminal already being committed, but the shutdown reason itself writes no lifecycle transition. Work that outlives a process remains active and follows the same level-triggered reconciliation path used after `kill -9`.
