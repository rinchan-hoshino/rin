...
- Chat owns `inbox_jobs`, messages, and platform outbox delivery only. It reconciles inherited running jobs before adapters start and accepted jobs continuously afterward, attaching them to the daemon request without submitting the prompt again. A claim lease coordinates execution only: a reconciler atomically replaces the owner epoch before taking over, so stale executors lose their SQLite fence; process death or lease expiry never becomes a business terminal. Every interactive frontend submits the same prompt RPC with Pi's native streaming behavior. Pi alone classifies the input as `prompt`, `steer`, or `followUp`; daemon and frontend state never infer that choice from Working, a ledger row, or a controller. The daemon durably opens terminal ownership only after Pi admits a lifecycle prompt. Steering persists its deterministic request identity in the Pi user entry before Chat settles that inbox job, and it never creates or transfers terminal ownership: the original accepted prompt still owns the single Pi terminal and final outbox. Chat does not revive, supersede, infer, or mirror Pi lifecycle state.
