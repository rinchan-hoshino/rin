...
10. keeps persisted-shape ownership in the installer: an atomic schema-authority sidecar lets install/update preflight and ordinary runtime reject unmarked legacy databases without opening or changing their main/WAL/SHM files, an upgrade with legacy search data fails closed if the staged runtime is unavailable, the stopped-service target-user migration marks its interrupted state, replaces the incompatible index set, and rebuilds schema v5 from canonical archives through that staged runtime, and only a completed rebuild publishes the `current` marker.
...
The memory-search tests cover Chinese and Latin text, structured identifiers, tool names, exact filtering, ranking, full-session archive loading, missing archives, installer-owned schema-v4 rebuild, side-effect-free migration preflight, byte-for-byte preservation of legacy main/WAL/SHM files during runtime rejection and preflight, rewritten archives, durable dirty-writer recovery, and interrupted/corrupt repair paths. Production rollout still requires the normal install/update transaction and must not migrate the live index from daemon startup or an ordinary query.
