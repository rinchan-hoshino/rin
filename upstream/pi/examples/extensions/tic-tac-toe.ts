...
function borderFgCode(
  adjacent: ReadonlyArray<{ cell: Cell; isWin: boolean }>,
): string | undefined {
  const fgs = adjacent
    .map((a) => cellFgCode(a.cell, a.isWin))
...
function renderBoard(opts: BoardRenderOpts): string[] {
  ...
  for (let gridR = 0; gridR <= 3; gridR++) {
    ...
    for (let gridC = 0; gridC <= 3; gridC++) {
      ...
      row += paintBorder(
        cornerChar(gridR, gridC),
        isCursorCorner(gridR, gridC),
        cornerColor,
      );
      if (gridC < 3) {
    ...
    for (let lineIdx = 0; lineIdx < CELL_HEIGHT; lineIdx++) {
      ...
      for (let gridC = 0; gridC <= 3; gridC++) {
        ...
        contentRow += paintBorder(
          "\u2502",
          isCursorVBorder(gridR, gridC),
          vColor,
        );
        if (gridC < 3) {
...
class TicTacToeComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    if (this.state.status !== "playing") {
      ...
      lines.push(
        centerPad(
          `${blue("X")} (You)  ${dim("|")}  Turn: ${bold(yellow("O"))} (Agent)`,
          width,
        ),
      );
    }
...
function reconstructState(ctx: ExtensionContext): void {
  ...
  for (const entry of ctx.sessionManager.getBranch()) {
    ...
    if (
      msg.toolName !== "tic_tac_toe" &&
      msg.toolName !== "tic_tac_toe_see_board"
    )
      continue;
...
`;
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "Play a tic-tac-toe action (move_up/down/left/right or play) as Player O",
    promptGuidelines: [
    ...
    parameters: Type.Object({
      action: StringEnum(
        ["move_up", "move_down", "move_left", "move_right", "play"] as const,
        {
          description:
    ...
    renderCall(args, theme) {
      ...
      return new Text(
        theme.fg("toolTitle", theme.bold("tic_tac_toe ")) +
  ...
  pi.registerTool({
    ...
    renderCall(_args, theme) {
      return new Text(
        theme.fg("toolTitle", theme.bold("tic_tac_toe_see_board")),
        0,
        0,
      );
    },
